...
- 🔬 **Rollups horaires `check_rollups_1h`** (plan V2 A-2) — agrégat par monitor et par heure (compteurs de
  ...
  par monitor. Branché sur les statistiques exposées depuis A-3 (voir ci-dessous)
- 🔬 **`stats.py` lit rollups + brut** (plan V2, A-3) — `compute_daily_history(_bulk)`,
  `compute_percentile_timeseries` et `compute_uptime_in_range` servent les heures couvertes par
  `check_rollups_1h` et le reste depuis le brut ; frontière dérivée (`rollup_boundary() = max(bucket) + 1h`,
  aucun knob à régler) — table de rollups vide retombe intégralement sur le brut, le comportement d'avant
  A-3. Exact sur toute largeur (compteurs, uptime consensus, avg/min/max) sauf le p95 au-delà d'une heure
  (moyenne des p95 horaires pondérée par les échantillons, exposé `p95_is_estimate` et affiché `≈` côté SLA) ;
  parité dispatch/rollups garantie par `tests/test_stats_rollup_parity.py`
- 🔬 **Rétention différenciée brut / rollups** (plan V2, A-4) — `DATA_RETENTION_DAYS` (90 j) ne régit plus
  que `check_results` ; `ROLLUP_RETENTION_MONTHS` (13 mois, 0 = infini) régit `check_rollups_1h`, en mois
  calendaires (pas `mois × 30 j`). **Interlock** : tous les cutoffs de purge du brut (drop de partition,
  `DELETE` global, `DELETE` par moniteur) sont plafonnés par `rollup_boundary()` — supprimer une ligne brute
  pas encore repliée la perdrait dans les deux tables. Pas d'interlock si `ROLLUP_ENABLED=false` (figerait le
  purge pour toujours) ou table de rollups encore vide (log `retention_no_rollup_floor` si ça persiste)
- 🔬 **`custom_metrics` partitionné aussi** (plan V2, C-2) — deuxième table pilotée par le temps
  (`PARTITION BY RANGE (pushed_at)`, PK composite `(id, pushed_at)`), même bascule sans copie que A-1 ;
  `core/partitions.py` généralisé (`PartitionSpec`) pour couvrir les deux tables partitionnées d'un seul
  module. Purge nightly `METRICS_RETENTION_DAYS` (90 j, 0 = infini, `purge_old_metrics`) — avant C-2 cette
  table n'était purgée nulle part. Pas encore de rollups pour les métriques (donc pas d'interlock)
