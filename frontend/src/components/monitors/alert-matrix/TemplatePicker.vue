<template>
  <div>
    <button
      type="button"
      @click="openModal"
      class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-(--border) text-xs text-(--text-2) hover:border-(--border-hover) hover:text-(--text-1) bg-(--bg-surface)"
    >
      <Sparkles :size="14" />
    ...
    <BaseModal v-model="open" :title="t('alert_matrix.templates.title')" size="md">
      <div class="space-y-3">
        <p class="text-xs text-(--text-3)">{{ t('alert_matrix.templates.hint') }}</p>

        <div v-if="loading" class="text-sm text-(--text-3) text-center py-6">
          {{ t('common.loading') }}
        </div>

        <div v-else-if="!templates.length" class="text-xs text-(--text-3) text-center py-6 italic">
        ...
        <div v-else class="space-y-2">
          <div
            ...
            :class="selectedId === tpl.id ? 'border-(--accent-border) bg-(--accent-glow)' : 'border-(--border) hover:border-(--border-hover)'"
          >
            <button type="button" @click="selectedId = tpl.id" class="w-full text-left p-3">
              <div class="flex items-start justify-between gap-2">
                <div class="min-w-0">
                  <div class="text-sm font-semibold text-(--text-1) flex items-center gap-2">
                    <span>{{ tpl.name }}</span>
                    <span
                      v-if="tpl.is_system"
                  ...
                  <p v-if="tpl.description" class="text-[11px] text-(--text-3) mt-0.5">
                    {{ tpl.description }}
                  </p>
                </div>
                <span class="shrink-0 text-[10px] text-(--text-3) uppercase tracking-wide">
        ...
        <div
          ...
          class="flex items-start gap-2 rounded-lg border border-[color-mix(in_srgb,var(--warn)_25%,transparent)] bg-[color-mix(in_srgb,var(--warn)_10%,transparent)] px-3 py-2 text-[11px] text-(--warn)"
        >
