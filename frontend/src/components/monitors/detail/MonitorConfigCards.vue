<template>
  ...
  <div v-if="hasNetworkScope" class="card mb-6">
    ...
    <p class="text-xs text-gray-500 mt-2">{{ patch.networkScopeOptions.value.find(s => s.value === monitor.network_scope)?.desc }}</p>
  </div>
    ...
    <template v-if="monitor.schema_drift_enabled">
      <div class="flex items-start justify-between gap-4">
        ...
        <div class="flex gap-2 flex-shrink-0">
          ...
          <button @click="patch.resetSchemaBaseline" :disabled="!monitor.schema_baseline" class="btn-ghost text-xs text-red-400 hover:text-red-300 disabled:opacity-50">Reset</button>
        </div>
      </div>
    </template>
    <template v-else>
  ...
  <div v-if="isComposite" class="card mb-6">
    ...
    <div v-if="deps.compositeMembers.value.length" class="space-y-2 mb-4">
      <div v-for="m in deps.compositeMembers.value" :key="m.id"
        ...
        <span class="flex-1 text-sm text-gray-300 font-mono">{{ deps.memberName(m.monitor_id) }}</span>
