<template>
  <div class="card">
    <h2 class="text-sm font-semibold text-(--text-2) mb-4">{{ t('monitor_detail.recent_checks') }}</h2>
    <table class="w-full text-sm">
      ...
      <tbody class="divide-y divide-(--border)">
        <tr v-for="r in results.slice(0, 50)" :key="r.id">
          ...
          <td v-if="!noHttpTypes.includes(monitor.check_type)" class="py-2 text-(--text-2)">
            {{ r.http_status ?? '—' }}
          </td>
          <td class="py-2 text-(--text-2)">
          ...
          <td v-if="isHttpLike" class="py-2 hidden xl:table-cell">
            <div v-if="r.ttfb_ms != null" class="flex items-center gap-1.5 text-xs font-mono min-w-[120px]">
              <div class="flex h-2 rounded overflow-hidden flex-1 bg-(--bg-surface-2)">
                <div
                  class="bg-(--accent-glow) h-full"
                  :style="`width:${Math.round((r.dns_resolve_ms || 0) / r.response_time_ms * 100)}%`"
                  title="DNS"
                ></div>
                <div
                  class="bg-[color-mix(in_srgb,var(--warn)_70%,transparent)] h-full"
                  :style="`width:${Math.round(r.ttfb_ms / r.response_time_ms * 100)}%`"
                  title="TTFB"
                ></div>
                <div class="bg-[color-mix(in_srgb,var(--up)_70%,transparent)] h-full" :style="`flex:1`" title="Download"></div>
              </div>
              <span class="text-(--text-3)">{{ r.ttfb_ms }}ms</span>
            </div>
          ...
          <td v-if="monitor.check_type === 'scenario'" class="py-2 text-xs">
            <span v-if="r.scenario_result">
              ...
              <span v-if="r.scenario_result.failed_step_label" class="text-(--text-3) ml-1">
          ...
          <td
            ...
            class="py-2 text-(--text-2) hidden md:table-cell"
          >{{ r.redirect_count }}</td>
          <td v-if="monitor.ssl_check_enabled" class="py-2 hidden lg:table-cell">
            <span v-if="r.ssl_valid === null || r.ssl_valid === undefined" class="text-(--text-3) text-xs">—</span>
            <span v-else-if="r.ssl_valid" class="text-xs text-(--up)">
              ✓ {{ r.ssl_days_remaining }}{{ t('monitor_detail.days_short') }}
            </span>
            <span v-else class="text-xs text-(--down)">✗ {{ t('monitor_detail.ssl_expired') }}</span>
          </td>
          <td
            v-if="noHttpTypes.includes(monitor.check_type)"
            class="py-2 text-xs text-(--down) hidden md:table-cell truncate max-w-xs"
          >{{ r.error_message || '—' }}</td>
        </tr>
      </tbody>
    </table>
