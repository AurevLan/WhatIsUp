...
every monitor. This module keeps: maintenance suppression bookkeeping, and the
post-decider side effects shared by both callers of ``process_check_result``
and ``open_incident_from_health`` (schema drift, anomaly detection,
...
async def process_check_result(
    ...
    Handles maintenance suppression bookkeeping and post-decider side
    effects. Availability incidents themselves are opened/resolved by
    ``services.health.evaluate_slos`` on every monitor (plan Cap v2 4b) —
    not here.
    """
    ...
    # (schema drift, anomaly, auto-pause) still run below regardless of
    # monitor.health_engine_enabled.
...
async def _post_decider_side_effects(
    ...
    Engine: schema drift, anomaly detection, auto-pause.
    """
    if monitor is None:
        return
